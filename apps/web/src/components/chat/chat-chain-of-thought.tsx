...
export function ChainOfThought({
	...
	const shouldShowHiddenReasoning =
		reasoningRequested && !reasoningText && !isStreaming &&
	...
	const shouldShowUnavailableReasoning =
		reasoningRequested && !reasoningText && !isStreaming &&
		!shouldShowHiddenReasoning && !shouldShowNoReasoningTokens;
	...
	if (!hasToolSteps) {
		if (!reasoningText && !isStreaming && !shouldShowUnavailableReasoning && !shouldShowHiddenReasoning && !shouldShowNoReasoningTokens) {
			return null;
		}
		return (
			<Reasoning
			...
			>
				<ReasoningTrigger
					getThinkingMessage={(streaming, duration) => {
						if (shouldShowUnavailableReasoning) return <p>Reasoning unavailable</p>;
						if (shouldShowHiddenReasoning) return <p>Reasoning hidden</p>;
						if (shouldShowNoReasoningTokens) return <p>No reasoning used</p>;
						if (streaming) return <p>Thinking...</p>;
						if (duration === undefined || duration === 0) return <p>Thought process</p>;
	...
	const getToolLabel = (step: ChainOfThoughtStepData) => {
		...
		if (step.toolState === "output-available") return `Search: ${query || step.toolName || "tool"}`;
		if (step.toolState === "output-error") return `Search failed: ${query || step.toolName || "tool"}`;
