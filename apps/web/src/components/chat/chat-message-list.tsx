...
import { ChainOfThought, buildChainOfThoughtSteps } from "./chat-chain-of-thought";
	...
	const processedMessages = useMemo(() => {
		...
		return messages.map((message) => {
			...
			const isCurrentlyStreaming = allParts.some((part) => {
				if ("state" in part && part.state === "streaming") return true;
			...
			const reasoningTokenCount =
				typeof metadata?.reasoningTokenCount === "number" ? metadata.reasoningTokenCount : undefined;
			...
			return {
				message, msg, textParts, fileParts, thinkingSteps, isAnyStepStreaming, hasTextContent,
				thinkingTimeSec, reasoningRequested, reasoningTokenCount, resumedFromActiveStream,
				isCurrentlyStreaming, shouldSkip,
			};
		});
	}, [messages]);
	...
	useEffect(() => {
		...
		setOpenByMessageId((prev) => {
			...
			for (const item of processedMessages) {
				...
				if (wasThinkingStreaming && hasStartedAnswerText && next[item.message.id] !== false) {
									...
									<Message from={item.message.role as "user" | "assistant"}>
										...
										{item.message.role === "user" ? (
											<UserMessageActions
												...
												onRetry={(modelId?: string) => { void onRetryMessage(item.message.id, modelId); }}
												onFork={(modelId?: string) => { void onForkMessage(item.message.id, modelId); }}
											/>
										) : (
