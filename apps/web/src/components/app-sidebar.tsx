...
export function AppSidebar({
  ...
  const grouped = useMemo(
    () => groupChatsByTime(chats, Date.now()),
    [chats, dayKey],
  )
  ...
  const handleModeToggle = () => {
    navigate({ to: isStudioRoute ? '/' : '/studio' })
  }
  const modeToggleTitle = isStudioRoute
    ? 'Switch to Chat'
    : 'Switch to Image Studio'
  const headerIconButtonClass =
    'flex size-9 items-center justify-center rounded-lg text-sidebar-foreground/70 transition-colors hover:bg-sidebar-accent hover:text-sidebar-foreground'
      ...
      <Sidebar variant={variant} collapsible={collapsible} {...props}>
        <div className="relative flex h-14 shrink-0 items-center justify-center px-3">
          <button
            ...
            className={cn('absolute left-3', headerIconButtonClass)}
            title="Close sidebar"
          >
            <SidebarIcon />
          </button>
          ...
          <button
            ...
            className={cn('absolute right-3', headerIconButtonClass)}
