 ...
 * When set, only x-forwarded-for values from requests originating behind
 * a known proxy are accepted. Platform-specific modes (cloudflare, vercel)
 * use tamper-resistant headers and do not require this setting.
 */
...
if (TRUST_PROXY_MODE === "true" && TRUSTED_PROXY_IPS.size === 0) {
	console.error(
		"[Models API] SECURITY: TRUST_PROXY=true is configured without TRUSTED_PROXIES. " +
			"The x-forwarded-for header can be spoofed by clients, bypassing rate limits. " +
			"Set TRUSTED_PROXIES to a comma-separated list of trusted reverse-proxy IPs, " +
			"or switch to a platform-specific mode: TRUST_PROXY=cloudflare or TRUST_PROXY=vercel. " +
			"Rate limiting will reject all requests until this is resolved.",
	);
}
...
function getClientIp(request: Request): string | null {
	...
	if (TRUST_PROXY_MODE === "true") {
		...
		// x-forwarded-for is only trusted when TRUSTED_PROXIES is configured,
		// indicating the operator has verified their reverse-proxy chain
