 ...
 * When empty, TRUST_PROXY=true will fail closed (reject requests)
...
if (TRUST_PROXY_MODE === "true" && TRUSTED_PROXY_IPS.size === 0) {
	console.warn(
		"[Models API] SECURITY WARNING: TRUST_PROXY=true without TRUSTED_PROXIES is unsafe. " +
			...
			"Rate limiting will fail closed and reject requests when client IP cannot be verified.",
	);
}
...
function getClientIp(request: Request): string | null {
	...
	if (TRUST_PROXY_MODE === "true") {
		...
		// Only fall back to x-forwarded-for when TRUSTED_PROXIES is configured,
		// since without a verified proxy chain this header is user-controlled.
